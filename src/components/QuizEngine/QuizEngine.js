...
const QuizEngine = () => {
    ...
    const { player, setScore, setTotalTime, setQuizLength } = useContext(QuizContext);
    ...
    }, [currentQuestionIndex, questions.length, answerTimes, localScore, navigate, setScore, setTotalTime, resetQuestionState]);
    ...
    useEffect(() => {
        ...
        setQuestions(loaded);
        setQuizLength(loaded.length);
        setIsLoading(false);
    }, [player, navigate, setQuizLength]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setTimer(prev => {
                if (prev === 1) {
                    setAnswerTimes(prevTimes => [...prevTimes, { question: currentQuestionIndex, time: 15 }]);
                    handleNextQuestion();
                    return 15;
                }
                return prev - 1;
    ...
    }, [currentQuestionIndex, isAnswered, questions.length, handleNextQuestion]);
